...
/**
 * ListUserTasks creates an HTML table that shows tasks assigned to a given user.
 */
class ListUserTasks extends \rocinante\controller\Command
{
   ...
   /**
    * Creates an HTML table that shows tasks assigned to a given user.
    */
   public function doExecute()
   {
      if ($this->request->getProperty('cmd') === "task/ListUserTasks")
      {
         ...
         $taskCounter->distinct()->count("TaskId")->field('UserId')->eq($userid);
         ...
         $taskIdentity->distinct()->field("UserId")->eq($userid)->leftJoin($esoTableIdentity, "TableId", "TableId")->join($userIdentity, "Task.AssignerId", "User.UserId");
         ...
         $html .= "<th style='width: 45%'>" . $tableL10n->description . "</th>\n";
         $html .= "<th>" . $tableL10n->count . "</th>\n";
         $html .= "<th>" . $tableL10n->progress . "</th>\n";
         $html .= "<th style='width: 12%'>" . $tableL10n->assigner . "</th>\n";
         $html .= "</tr>\n";
         $html .= "</thead>\n";
         $html .= "<tbody>\n";
         $rowCounter = 0;
         ...
         if ($rowCounter === 0)
         {
            $html  = "<tbody>\n";
            $html .= "<tr>\n";
            $html .= "<td class='ui-widget-content' style='width: 100%; text-align: center'>" . $this->l10n->{"frontpage"}->{"tabs"}->{"tasks"}->{"no-tasks"} . "</td>\n";
            $html .= "</tr>\n";
         }
         $html .= "</tbody>\n";
